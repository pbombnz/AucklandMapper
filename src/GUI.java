...
public abstract class GUI {
	...
	public enum AStar {
		STANDARD, DISTANCE, BOTH, NONE
	};

	public enum APoint {
		RECURSIVE, ITERATIVE, BOTH, NONE
	};
	...
	/**
	 * Is called when the mouse's scroll wheel is active and is passed the MouseEvent 
	 */
	protected abstract void onMouseWheelMoved(MouseWheelEvent e);
	/**
	 * Is called when the mouse dragged is active and is passed the MouseEvent 
	 */
	...
	protected abstract void onAStarToggle(AStar method);
	
	protected abstract void onAPointToggle(APoint method);
	...
	/**
	 * @return Whether the mouse input is ready for A* search or just simple mouse click
	 */	
	public boolean isAStarSelection() {
		return isAStarSelection;
	}
	
	public boolean isAPointSelection() {
		return isAPointSelection;
	}
	
	//public JPanel getNavigationBar() { return (JPanel) controls.getComponent(2); }
	protected JPanel getAdditionalButtonsPanel() {
		return controls_additionalButtons;
	}
	...
	private static final int DEFAULT_DRAWING_WIDTH = 700;
	...
	private JPanel controls_additionalButtons;
	...
	private boolean isAStarSelection;
	private boolean isAPointSelection;
	...
	private void initialise() {
            ...
            public void actionPerformed(ActionEvent ev) {
                ...
                if (fileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
                    ...
                    if (nodes == null || roads == null || segments == null) {
                        ...
                        isAPointSelection = false;
                        isAStarSelection = false;
                        for(int i = 0; i < getAdditionalButtonsPanel().getComponentCount(); i++) {
                        	if( getAdditionalButtonsPanel().getComponent(i) instanceof JButton) {
                        		JButton button = (JButton) getAdditionalButtonsPanel().getComponent(i);
        ...
        final JButton aPoint = new JButton("A. Pt. OFF");
        final JButton aStar = new JButton("A* SEL. OFF");
        
        aStar.setEnabled(false);
        aStar.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                isAStarSelection = !isAStarSelection;
                isAPointSelection = false;
                aPoint.setText("A. Pt. OFF");
                JButton thisButton = (JButton) ev.getSource();
                AStar method = AStar.NONE;
                if(isAStarSelection) {
                	thisButton.setText("A* SEL. ON");
                    
                	int option = JOptionPane.showOptionDialog(frame, "What Method would you like to use?", 
                       "Articulation Point Method", 0, JOptionPane.QUESTION_MESSAGE, 
                       null, new String[] {"Standard", "Distance Accounted", "DEBUG (Both)" }, 0);
                	
                    if(option == 0) {
                    	method = AStar.STANDARD;
                    } else if(option == 1) {
                    	method = AStar.DISTANCE;
                    } else if(option == 2) {
                    	method = AStar.BOTH;
                    }
                } else {
                	thisButton.setText("A* SEL. OFF");
                }
                onAStarToggle(method);
                redraw();
            }
        });

        aPoint.setEnabled(false);
        aPoint.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                isAPointSelection = !isAPointSelection;
                isAStarSelection = false;
                aStar.setText("A* SEL. OFF");
                JButton thisButton = (JButton) ev.getSource();
                APoint method = APoint.NONE;
                if(isAPointSelection) {
                	thisButton.setText("A. Pt. ON");
                    
                	int option = JOptionPane.showOptionDialog(frame, "What Method would you like to use?", 
                       "Articulation Point Method", 0, JOptionPane.QUESTION_MESSAGE, 
                       null, new String[] {"Recursive", "Iterative", "DEBUG (Both)" }, 0);
                    if(option == 0) {
                    	method = APoint.RECURSIVE;
                    } else if(option == 1) {
                    	method = APoint.ITERATIVE;
                    } else if(option == 2) {
                    	method = APoint.BOTH;
                    }
                } else {
                	thisButton.setText("A. Pt. OFF");
                }
                onAPointToggle(method);
                redraw();
            }
        });
        ...
        controls_additionalButtons = new JPanel();
        controls_additionalButtons.setMaximumSize(new Dimension(300, 60));
        controls_additionalButtons.setLayout(new GridLayout(2, 2));
        controls_additionalButtons.add(aPoint);
        controls_additionalButtons.add(aStar);
        controls.add(controls_additionalButtons);
        ...
        frame = new JFrame("Mapper (By Prashant Bhikhu)");
